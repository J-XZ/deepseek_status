...
/// 基于 LevelDB C API 的 Codex 用量历史存储。
/// actor 串行化所有访问，LevelDB 句柄在应用生命周期内只打开一次。
/// 与余额历史使用不同的目录，避免两个实例同时打开同一数据库。
actor LevelDBCodexHistoryStore: CodexHistoryStoring {
  ...
  static func open(
  ...
  ) throws -> LevelDBCodexHistoryStore {
    ...
    return LevelDBCodexHistoryStore(
      directory: directory,
  ...
  func upsert(samples: [CodexUsageSample], credentialID: String) async throws {
    ...
    for sample in samples {
      ...
      let key = CodexHistoryKeyCodec.key(
        credentialID: credentialID,
        bucketStart: sample.bucketStart
      )
      let value: Data
      do {
        value = try CodexHistoryValueCodec.encode(sample: sample)
      } catch {
  ...
  func fetch(credentialID: String, from: Date, to: Date) async throws -> [CodexUsageSample] {
    ...
    let prefix = CodexHistoryKeyCodec.credentialPrefix(credentialID: credentialID)
    let fromSeconds = Int64(from.timeIntervalSince1970)
    let toSeconds = Int64(to.timeIntervalSince1970)
    let seekKey = prefix + CodexHistoryKeyCodec.padded(fromSeconds)
    ...
    while dependencies.iteratorValid(iterator) != 0 {
      ...
      if let parsed = CodexHistoryKeyCodec.parse(key: key),
        ...
        parsed.bucketSeconds <= toSeconds
      {
        ...
        if let valuePointer = dependencies.iteratorValue(iterator, &valueLength) {
          ...
          if let sample = try? CodexHistoryValueCodec.decode(value),
  ...
  func prune(before: Date) async throws {
    ...
    let keys = try collectKeys(matching: CodexHistoryKeyCodec.schemaPrefixKey()) { key in
      guard let parsed = CodexHistoryKeyCodec.parse(key: key) else { return false }
  ...
  func deleteHistory(credentialID: String?) async throws {
    guard db != nil else { throw LevelDBError.notOpen }
    let prefix =
      credentialID.map(CodexHistoryKeyCodec.credentialPrefix)
      ?? CodexHistoryKeyCodec.schemaPrefixKey()

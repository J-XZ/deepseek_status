...
/// Cursor 历史样本生成、读写与过期清理。LevelDB 细节不暴露给调用方。
struct CursorHistoryService: Sendable {
  let store: any CursorHistoryStoring
  let clock: any DateProviding
  private let pruneGate: PruneGate

  init(store: any CursorHistoryStoring, clock: any DateProviding) {
    self.store = store
    self.clock = clock
  ...
  func save(sample: CursorUsageSample) async throws {
    try await store.upsert(samples: [sample], credentialID: sample.credentialID)
  }

  func recentSamples(credentialID: String, hours: Int = 72) async throws -> [CursorUsageSample] {
    let now = clock.now()

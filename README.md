...
纯 Swift 原生 macOS 菜单栏应用，实时显示 DeepSeek API 剩余余额、OpenCode Zen 余额与 OpenCode Go 用量，记录最近 14 天的变化趋势，并展示 DeepSeek 官方服务状态。常驻系统顶部菜单栏，不显示 Dock 图标。唯一第三方依赖为 Google LevelDB（Git submodule，见下文）。

## 界面截图

弹出窗口展示余额、官方服务状态和最近 14 天趋势：
...
- 启动自动刷新、每 30 秒自动刷新、打开菜单时距上次成功超过 60 秒自动刷新
...
- OpenCode Go 未订阅时使用红色提示进度条；已订阅时展示 5 小时、每周和月度用量窗口，并按理想进度自动显示蓝色、绿色或橘色，同时显示每个窗口的已用量、与理想用量的差异、重置时间和红色理想用量标记
- OpenCode 菜单栏使用两行显示 Go 本月剩余百分比及其与理想用量的差异、Zen 余额；无 Go 订阅或月度窗口不可用时第一行显示 `--`
...
4. 保存密钥后，应用会自动获取余额和官方服务状态。点击菜单栏项目可以查看总余额、充值余额、赠送余额、更新时间和最近 14 天趋势；点击底部「刷新」可立即更新。
...
7. OpenCode 用量页面会分别展示 Zen 余额和 Go 订阅状态。没有 Go 订阅时显示明确的红色未订阅提示；有订阅时显示 5 小时、每周、月度窗口的已用量、剩余量、与各自理想用量的差异、红色理想用量标记和重置时间。菜单栏第一行显示 Go 本月剩余百分比及理想差异，第二行显示 Zen 余额；历史趋势同样只保存在本机。
...
- 趋势图右上角会根据有效消耗速度估算额度耗尽时间：优先使用最近 1 小时，无消耗时依次回退到最近 24 小时、72 小时和全部历史；如果全部历史都没有余额下降，则明确显示“暂无足够用量数据”，不伪造时间。
- DeepSeek 显示余额耗尽估算；Codex 与 Cursor 显示周额度耗尽估算；OpenCode 有 Go 订阅时显示周额度与 Zen 余额耗尽估算，无订阅时只显示 Zen 余额耗尽估算（不估算月额度）。
...
- 趋势图只使用 `LineMark` 绘制连续数据，不绘制容易造成视觉噪声的小圆点；数据缺口超过 20 分钟时断开连线，14 天 10 分钟粒度（最多约 2016 桶）下依然流畅。
- X 轴 domain 明确为 `now - 14 天 ... now`，时间来源可注入。
- 历史只保存在本机，最多保留最近 14 天；应用启动时执行一次清理（使用注入时钟），之后按节流策略清理（只有清理成功后才会记录节流时间，失败后下次可重试）。
- 数据库位置：`<Application Support>/com.jxz.deepseekbalance/BalanceHistory.leveldb`；升级时会兼容读取旧的 `com.example.DeepSeekBalance` 目录。
- 设置区提供「清除本地历史」按钮（带确认对话框），只清除当前账号对应的历史，不影响 API Key 与当前余额；清除后当前余额中真实存在的币种仍保留在币种选择器中。
...
- OpenCode Zen 使用美元余额；OpenCode Go 使用独立的 5 小时、每周和月度订阅用量窗口。每个窗口以已用百分比和剩余百分比展示，并显示“实际已用量 − 理想已用量”的有符号差异、红色理想用量标记以及接口返回的重置时间。
...
- Go 已订阅时，趋势区域将 5 小时、每周、月度 Go 曲线和 Zen 余额曲线放在同一张图中：左侧 Y 轴为 Go 剩余百分比，右侧 Y 轴为 Zen 美元余额，底部图例标识每条曲线。
- OpenCode 趋势使用最近 14 天的 10 分钟本地样本；Go 与 Zen 共用时间轴，缺少数据时不会生成虚假样本。清除本地历史不会删除保存的 Cookie。
...
- 应用启动后立即获取官方状态，之后每 30 秒自动刷新；弹出菜单打开时缓存超过 60 秒则刷新；app wake / didBecomeActive 后按缓存年龄刷新；提供独立「刷新服务状态」按钮；底部「刷新」并发刷新余额与官方状态，两者错误互不覆盖。
...
- 首次启动默认使用简体中文；升级时未标记为用户主动选择的旧 English 默认值会迁移为中文。用户仍可切换到 English，选择持久化在 `UserDefaults`（仅语言偏好，API Key 仍只存 Keychain）。

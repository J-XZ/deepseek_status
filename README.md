...
- OpenCode 菜单栏使用两行显示 Go 本月已用百分比及其与理想用量的差异、Zen 余额；无 Go 订阅或月度窗口不可用时第一行显示 `--`
...
7. OpenCode 用量页面会分别展示 Zen 余额和 Go 订阅状态。没有 Go 订阅时显示明确的红色未订阅提示；有订阅时显示 5 小时、每周、月度窗口的已用量、剩余量、与各自理想用量的差异、红色理想用量标记和重置时间。菜单栏第一行显示 Go 本月已用量及理想差异，第二行显示 Zen 余额；历史趋势同样只保存在本机。
